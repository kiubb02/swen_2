...
import components.stats.StatsRequests;
...
public class App implements ServerApp {
    ...
    private final StatsRequests statsReq = new StatsRequests();
    ...
    public Response handleRequest(Request request) throws ParseException, SQLException, IOException, InterruptedException {
        ...
        if(authorized) { //if token is correct we can continue
            ...
            switch (req[1]) {
                ...
                // Battle Logic Start
                case "stats" -> res = this.statsReq.handleRequest(request);
                case "score" -> res = this.scoreReq.handleRequest(request);
                case "battles" -> res = this.battleReq.handleRequest(request);
                //Battle Logic End
